...
-opaque set(CVS) :: {set, gleam@dict:dict(CVS, list(nil))}.
...
-spec contains(set(CWD), CWD) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CWF), CWF) -> set(CWF).
delete(Set, Member) ->
...
-spec fold(set(CWO), CWQ, fun((CWQ, CWO) -> CWQ)) -> CWQ.
...
-spec filter(set(CWR), fun((CWR) -> boolean())) -> set(CWR).
...
-spec drop(set(CWY), list(CWY)) -> set(CWY).
drop(Set, Disallowed) ->
...
-spec take(set(CXC), list(CXC)) -> set(CXC).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 290).
-spec order(set(CXK), set(CXK)) -> {set(CXK), set(CXK)}.
order(First, Second) ->
...
-spec intersection(set(CXP), set(CXP)) -> set(CXP).
...
-spec difference(set(CXT), set(CXT)) -> set(CXT).
...
-spec is_subset(set(CXX), set(CXX)) -> boolean().
...
-spec is_disjoint(set(CYA), set(CYA)) -> boolean().
...
-spec each(set(CYH), fun((CYH) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CWA), CWA) -> set(CWA).
insert(Set, Member) ->
...
-spec from_list(list(CWL)) -> set(CWL).
from_list(Members) ->
...
-spec map(set(CWU), fun((CWU) -> CWW)) -> set(CWW).
map(Set, Fun) ->
...
-spec union(set(CXG), set(CXG)) -> set(CXG).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(CYD), set(CYD)) -> set(CYD).
