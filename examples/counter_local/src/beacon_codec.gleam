...
pub fn encode_model(
  state: #(counter_local.Model, counter_local.Local, Nil),
) -> String {
  let model = state.0
  let local = state.1
  json.object([
...
pub fn decode_model(
  json_str: String,
) -> Result(#(counter_local.Model, counter_local.Local, Nil), String) {
  let state_decoder = {
    use count <- decode.field("count", decode.int)
    use input <- decode.field("input", decode.string)
    use menu_open <- decode.field("menu_open", decode.bool)
    decode.success(#(
      counter_local.Model(count: count),
      counter_local.Local(input: input, menu_open: menu_open),
      Nil,
    ))
  }
...
pub fn decode_event(
  ...
  data: String,
  _target_path: String,
) -> Result(counter_local.Msg, String) {
      ...
      case tag {
        ...
        "ToggleMenu" -> {
          ...
          case json.parse(json_str, msg_decoder) {
            ...
            Error(_) ->
              Error(
                "Generated Beacon message payload did not match tag ToggleMenu",
              )
          }
        }
