...
pub fn decode_model(
  json_str: String,
) -> Result(#(dashboard.Model, Nil, Nil), String) {
  let state_decoder = {
    ...
    use process_history <- decode.field(
      "process_history",
      decode.list(decode.int),
    )
    use memory_history <- decode.field(
      "memory_history",
      ...
      case tag {
        "RefreshStats" -> {
          ...
          case json.parse(json_str, msg_decoder) {
            ...
            Error(_) ->
              Error(
                "Generated Beacon message payload did not match tag RefreshStats",
              )
          }
        }
