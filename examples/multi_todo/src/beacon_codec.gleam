...
pub fn decode_model(
  json_str: String,
) -> Result(#(app.Model, app.Local, Nil), String) {
  let state_decoder = {
    use todos <- decode.field(
      "todos",
      decode.list(server_decode_task_todoitem()),
    )
    ...
    decode.success(#(
      app.Model(todos: todos, input: input, next_id: next_id),
      app.Local(filter: server_decode_task_filter_value(filter)),
      Nil,
    ))
  }
