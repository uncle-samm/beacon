...
fn server_decode_auth_user() -> decode.Decoder(auth.User) {
  ...
  decode.success(auth.User(
    name: name,
    email: email,
    role: server_decode_auth_role_value(role),
  ))
}
...
pub fn decode_model(json_str: String) -> Result(#(app.Model, Nil, Nil), String) {
  let state_decoder = {
    ...
    decode.success(#(
      app.Model(user: user, items: items, next_id: next_id, input: input),
      Nil,
      Nil,
    ))
  }
      ...
      case tag {
        ...
        "ToggleItem" -> {
          ...
          case json.parse(json_str, msg_decoder) {
            ...
            Error(_) ->
              Error(
                "Generated Beacon message payload did not match tag ToggleItem",
              )
          }
        }
        "SetRole" -> {
          ...
          case json.parse(json_str, msg_decoder) {
            ...
            Error(_) ->
              Error(
                "Generated Beacon message payload did not match tag SetRole",
              )
          }
        }
